...
- **Verifikasi WebSocket QRIS Mandiri**: Mengintegrasikan kembali sistem notifikasi real-time WebSocket (`triggerWebSocketPaidEvent`) pada endpoint verifikasi [`qris_verify.php`](file:///D:/mikhmonv3ws/Mikhmon%20Server/mikhmon/qris_verify.php). Browser pelanggan akan langsung ter-refresh instan saat pembayaran berhasil dikonfirmasi.
- **Integrasi Inisialisasi WebSocket Client**: Memperbaiki pemanggilan fungsi client-side di [`frontpage.php`](file:///D:/mikhmonv3ws/Mikhmon%20Server/mikhmon/frontpage.php) dengan mengaktifkan langganan event Pusher (`initWebSocketConnection`) untuk menyambungkan koneksi socket peramban pelanggan ke Pusher/Soketi.
- **Sinkronisasi Ambang Batas Pemulihan Transaksi**: Memperbaiki pemulihan transaksi aktif pending di [`frontpage.php`](file:///D:/mikhmonv3ws/Mikhmon%20Server/mikhmon/frontpage.php) dengan menyelaraskan ambang batas kadaluwarsa dari 900 detik (15 menit) menjadi 300 detik (5 menit) demi mencegah loop modal kedaluwarsa instan pada browser pelanggan.
